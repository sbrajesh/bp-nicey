<div id="userbar">
	<h3><?php _e( 'Me', 'bpmag' ) ?></h3>

	<?php do_action( 'bp_user_bar_before' ) ?>
	
	<?php if ( is_user_logged_in() ) : ?>
		
		<?php if ( function_exists('bp_loggedin_user_avatar') ) : ?>
		<p class="avatar">
	...
	<?php else : ?>
		...
		<p id="login-text"><?php _e( 'You must log in to access your account.', 'bpmag' ) ?></p>
		...
		<form name="loginform" id="loginform" action="<?php echo site_url('wp-login.php') ?>" method="post">
			...
			<p class="forgetmenot"><label><input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <?php _e( 'Remember Me', 'bpmag' ) ?></label></p>
			<p class="submit">
				<input type="submit" name="wp-submit" id="wp-submit" value="<?php _e('Log In',"bpmag"); ?>" tabindex="100" />
